...
```{r}
# Henter lenke fra ssb, og gjør en query.
lenke <- "https://data.ssb.no/api/v0/no/table/07967"
...
# Gjør "år" om fra chr til dbl.
df$år <- as.numeric(df$år)

# Endrer verdi fra "million" til "milliard".
df <- df %>%
  mutate(verdi = verdi/1000)

# Sjekker at alt er slik jeg ønsket det.
df
...
```{r}
# Lager en ny df, deretter filtrerer år mellom 2015 til 2020, sysselsatte, næring og type var.
df1 <- df %>%
  filter(between(år, as.Date.numeric('2015'), as.Date.numeric('2020'))) %>%
  filter(gruppe %in% c("20-49 sysselsatte", "50-99 sysselsatte", "100-199 sysselsatte", "200-499 sysselsatte", "500 sysselsatte og over")) %>%
  filter(næring %in% c("Industri", "Tjenesteyting", "Andre næringer")) %>%
  filter(var %in% c("FoU-kostnader i alt"))

# Endrer navn på næring til y.
df1 <- df1 %>%
  mutate(y = næring)
...
```{r}
# Lager ny df, har samme filter som oppgaven over, men legger også inn de nye "var".
df2 <- df %>%
  filter(between(år, as.Date.numeric('2015'), as.Date.numeric('2020'))) %>%
  filter(gruppe %in% c("20-49 sysselsatte", "50-99 sysselsatte", "100-199 sysselsatte", "200-499 sysselsatte", "500 sysselsatte og over")) %>%
  filter(næring %in% c("Industri", "Tjenesteyting", "Andre næringer")) %>%
  filter(var %in% c("FoU-kostnader i alt", "Lønnskostnader", "Kostnader til innleid personale", "FoU-investeringer (kapitalkostnader)", "Andre driftskostnader"))

# Sjekker at det kom gjennom.
df2

# Plotter på samme vis som ovenfor.
df2 %>%
  ggplot(aes(x = verdi, fill = gruppe)) +
