title: “SOK-1004, høst 2022, Case 1”
...
```{r}
# Her lager jeg en tibble av dataen, som ligger i "dataset", i df.
df1 <- as_tibble(df$dataset)
# Endrer så Tid fra <chr> til <int>
df1 <- df1 %>%
  mutate(Tid = as.integer(Tid))
#Til slutt så endrer jeg navnet på de forskjellige kollonenne.
df1 <- df1 %>%
...
```{r}
# Måtte endre fra "df" til "df1", siden jeg kalte tibble´n min "df1".
df1 %>%
  filter(var == "BNP") %>%
...
```{r}
# Endrer først verdi slik at den viser hele tusen, deretter sier jeg at jeg skal ha "BNP" og "MEMOBNP" med inn i grafen, og spesifiserer at tidsrammen er etter år 2000.
# Lager så grafen med hjelp av ggplot, og til sist så endrer jeg navnene på overskriften, og aksene. 
df1 %>%
  mutate(verdix = verdi/100) %>%
  filter(var %in% c("BNP", "MEMOBNP"), tid >=2000) %>%
  ggplot(aes(x = tid, y = verdix, color = var))  + geom_line() + theme_minimal() + xlab("År") + ylab("Kr i hele tusen") + ggtitle("BNP per innbygger")
```

*Inntektene har økt på begge grafene, når vi ser på "BNP" så ser det ut som det er en kraftig økning i inntektene til folk, mens på "MEMOBNP" er det en mer moderat vekst. Dette kommer av at man har inflasjon, og det ser man spesielt i år-skiftet 2020-2021, dette har med pandemien og stor inflasjon å gjøre. Forskjellen på bnp i løpende og fast kurs er at på løpende så ser man BNP fra den tids penger, men med fast så kan man sammenligne fra dagens penger, dette gjør at man kan få et bedre innblikk i penge statusen i de forskjellige årene. formålet det er mest relevant for er hvis man skal sammenligne mellom årene.*
...
*Denne koden laget en liste som er sammenlignbar side til side, den setter BNP og MEMOBNP ved siden av hverandre, mens df1 har først hele listen til BNP, og under der kommer MEMOBNP. Hvis man skal sammenligne BNP fra løpende eller fast kurs uten en graf så er df1_wide det beste alternativet*
...
ggplot(df1_long, aes(x = tid, y = verdi, color = var)) + 
  geom_point() + 
  geom_smooth() + 
