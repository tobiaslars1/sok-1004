title: “SOK-1004, høst 2022, Case 2”

author: “37”

format: pdf

echo: true

output: true

editor: visual


## Instruksjoner

Denne oppgaven skal løses interaktivt i RStudio ved å legge inn egen kode og kommentarer. Det ferdige dokumentet lagres med kandidatnummeret som navn `[kandidatnummer]_SOK1004_C2_H22.qmd` og lastes opp på deres GitHub-side. Hvis du har kandidatnummer 43, så vil filen hete `37_SOK1004_C2_H22.qmd`. Påse at koden kjører og at dere kan eksportere besvarelsen til pdf. Lever så lenken til GitHub-repositoriumet i Canvas.

## Bakgrunn

Økonomisk vekst gir økt kjøpekraft og velferd. Økningen i økonomisk aktivitet har hittil blitt muliggjort gjennom å utnytte fossile energikilder som kull, olje og gass. Forbrenningen av hydrokarboner har over tid økt mengden CO2 i atmosfæren og forårsaket endringer i klimaet. Klimaendringene medfører en rekke kostnader, blant andre hyppigere og mer intense innslag av ekstreme værforhold. I den grad veksten drives av fossile energikilder er det ikke opplagt at høyere økonomisk aktivitet vil øke menneskelig velferd på sikt. Vi står ovenfor en avveining mellom økt kjøpekraft og klimaendringer.

I denne oppgaven skal vi utforske avveiningen mellom kjøpekraft og klimaendringer ved å studere sammenhengen mellom CO2-utslipp og bruttonasjonalprodukt (BNP) på tvers av land. Data henter vi fra [OWID](https://ourworldindata.org/co2-emissions). En beskrivelse av dataene (kodebok) finnes [her](https://github.com/owid/co2-data/blob/master/owid-co2-codebook.csv).

Vi skal utforske følgende: Hvilke land har høyest CO2 utslipp? Hvordan kan utslippene sammenlignes når vi tar hensyn til befolkningen og størrelsen på økonomiene? Kan bruken av kull til energiproduksjon forklare forskjellen mellom land? Hvordan stiller utslippene seg når vi justerer for internasjonal handel og skiller mellom produksjon og forbruk?

**Merknad.** I oppgaven definerer jeg variabler matematisk og ber dere lage figurer av de. Alle variablene finnes i datasettet. En del av oppgaven er å finne de.

## Last inn pakker

```{r}
rm(list=ls())
library(tidyverse)
```

## Last ned data

```{r}
url <- 
  "https://raw.githubusercontent.com/owid/co2-data/master/owid-co2-data.csv"

df <- url %>%
  read_csv()
```

## I. Karbonintensitet i produksjon

Vi skal studere sammenhengen mellom BNP og CO2 for de fem største økonomiene i verden, målt ved kjøpekraftjustert BNP. Disse landene er:

```{r}
list <- c("China", "United States", "India", "Japan", "Germany")
```

Betrakt følgende figur:

```{r}
df %>%
  
  mutate(gdp = gdp/10^12) %>%

  mutate(co2 = co2/10^3) %>%

  filter(country %in% list) %>% 
  
  ggplot(aes(x=gdp, y=co2, color = country)) %>%
  
  + geom_point() %>%
  
  + geom_smooth(se = FALSE) %>%
  
  + labs(y = "CO2", x = "GDP", title = "GDP vs CO2", subtitle = "Production-based C02 emissions, billions of tons. 
GDP in trillions of USD, constant 2011 prices at purchasing power parity. 1850-2020.", color = "Country") %>%
  
  + theme_bw()
```

Kommandoen `geom_smooth()` bruker en såkalt lokal regresjon til å tegne en kurve gjennom datapunktene. Dere kan tenke på dette som en gjennomsnittlig sammenheng, eller at man har tegnet en strek gjennom datapunktene med en penn.

### Oppgave Ia

Gi en fullstendig og nøyaktig beskrivelse av figuren. Tolk helningen på linjene og diskuter variasjon mellom og innad i landene. Drøft hvilke forhold figuren illustrerer på en god måte og hvilken informasjon som er vanskelig å tolke.

*Denne figuren viser GDP og CO2 (Production-based C02 emissions) til USA, CHINA, INDIA, GERMANY og JAPAN i tidsrammen 1850-2020. Figuren gir et innblikk i sammenhengen mellom lands BNP og co2 utslipp. Man kan se at land med høy BNP også har høyt co2 utslipp. Men man ser også at både USA og CHINA har høy bnp men kinas co2 er en del høyere enn Usa´s co2 utslipp. Co2 utslippene måles i produksjonen til landet, dette gir oss en forklaring på hvorfor land som CHINA har høyest utslipp, landet er den største kull produsenten i verden, som vi ser i en graf lengere ned i oppgaven. USA sin co2 trend har vært postitv, det var en utflating på 1900-tallet, dette kan grunnes ved at USA har gått mer over til elektrisitet og fornybar energi, mens CHINA har fortsatt med mye kull.*

### Oppgave Ib

Vi betrakter nå et normalisert mål på sammenhengen CO2-utslipp og inntekt. La $CO2_{t}$ og $BNP_{t}$ være henholdsvis årlige utslipp av CO2 og brutto nasjonalprodukt i år $t$. Vi måler $CO2_{t}$ i kilogram og $BNP_{t}$ i kjøpekraftjusterte USD ved faste 2011-priser. Vi kan definere karbonintensiteten i produksjon som $CI_{t}=CO{2}_{t}/BNP_{t}$, altså antall kilogram CO2 per USD i verdiskapning.

Lag en figur som viser karbonintensiteten $CI_{t}$ til de fem største økonomiene (navnene er lagret i variabelen `list` over) fra 1850 til 2020. Sammenlign $CI_{t}$ med helningen fra figuren i oppgave Ia. Gi en fullstendig og nøyaktig beskrivelse av figuren. Drøft minst to forhold som du tror kan være av avgjørende betydningen for å forklare variasjonen innad og mellom land.

```{r}
# Løs oppgave Ib her 
df %>%
  
  mutate(gdp = gdp/10^12) %>%
  
  mutate(ci = co2/gdp) %>%
  
  mutate(ci = ci/10^3) %>%

  filter(country %in% list) %>% 
  
  ggplot(aes(x=year, y=ci, color = country)) %>%
  
  + geom_point() %>%
  
  + geom_smooth(se = FALSE) %>%
  
  + xlim(1850, 2020) %>%
  
  + labs(y = "Carbon intensity", x = "Years", title = "The carbon intensity", subtitle = "The carbon intensity (billions of tons) - 1850-2020.", color = "Country") %>%
  
  + theme_bw()
  
```

*Denne figuren viser karbonintensiteten til de fem landene i årene mellom 1850 til 2020. karbonintensitet er et mål for hvor mye klimagasser et land slipper ut, denne er en utregning mellom co2 og bnp til landet. som man ser på figuren så har land som Tyskland og Usa en enorm økning helt frem til ca 1900-tallet og deretter falater det ut frem til ca 1930 og etter det så synker de nedover. Oppgangen til Usa og Tyskland kan forklares grunnet den kjappe ekspanederigen i industien i disse landene. Alle landene har har en nedgang i karbonintensitet til dags dato, Kina har den høyeste karbonintensitet per dags dato. Kinas karbonintensitet kan forklares av deres store produksjon, samt kull. Nedgangen til landene kan skylles bruk av elektrisitet og fornybar energi*

### Oppgave Ic

Undersøk hvorvidt bruken av kull som energikilde kan forklare forskjellen i karbonintensitet mellom de fem største landene. Lag to figurer og 100-200 ord med tekst. Husk å gi en fullstendig og nøyaktig beskrivelse av figurene.

**Figur 1**

```{r}
# Figur 1 "Kull"
df %>%
  
  mutate(coal_co2 = coal_co2/10^3) %>%

  filter(country %in% list) %>% 
  
  ggplot(aes(x=year, y=coal_co2, color = country)) %>%
  
  + geom_point() %>%
  
  + geom_smooth(se = FALSE) %>%
  
  + xlim(1850, 2020) %>%
  
  + labs(y = "Coal emissions", x = "Years", title = "Coal emissions", subtitle = "Coal emissions (billion tons) 1850-2020", color = "Country") %>%
  
  + theme_bw()

# Figur 2 "co2/bnp"
df %>%
  
  filter(country %in% list) %>%
  
  ggplot(aes(x = year, y = co2_per_gdp, color = country)) %>%
  
  + geom_point() %>%
  
  + geom_smooth(se = FALSE) %>%
  
  + xlim(1850, 2020) %>%
  
  + labs(y = "Carbon intensity", x = "Years", title = "The carbon intensity", subtitle = "CO2 / GDP - 1850-2020.", color = "Country") %>%
  
  + theme_bw()
  


```

*Disse to figurene viser litt forskjellige ting, figur 1 viser bruken av kull til de fem landene over en tidsramme 1850 til 2020. Figur 2 er samme figur som lengere opp i oppgaven, og denne viser karbonintensiteten til de samme landene over samme tidsperiode. Figur 1 viser at Kina har desidert mest kull-utslipp, dette kan forklare den store forskjellen på co2 utslippene mellom kina og usa selvom deres bnp er nok så lik. Kullen til Kina kan også være med å forklare hvorfor Kina får en økt karbonintensitet. På figur 1 så ser vi at Japan og Tyskland har en stabil lav økning ved bruk av kull, mens Usa og India har en litt brattere kurve på denne, Usa har økning over lengere tid, og India har en litt kraftigere økning fra ca 1965.*

## II. Konsumbaserte CO2-utslipp

I den foregående analysen har vi slått fast at Kina har høy karbonintensitet i produksjon og at kullforbrenning står for en høy andel av deres utslipp. I hvilken grad kan utslippene knyttes til eksportrettet produksjon? I dataene fra OWID har de variable som de kaller konsumbaserte utslipp, for eksempel variabelen `consumption_co2.`

### Oppgave IIa

Forklar hvordan konsumbaserte CO2-utslipp er konstruert og hvordan vi skal tolke den.

*Konsumbaserte CO2-utslipp er i følge "ourworldindata.org", målt på produsjon, og disse er da også justert til "trade" eller handel. for å kalkulere disse må vi få en oversikt over varer som er handlet mellom landene i verden. Når man importerer varene må man inkludere alle co2-utslippene som ble til når det ble produsert, og omvendt når man eksporterer varer. konsumbaserte co2-utslipp reflekterer konsum og livstil til landets innbyggere.*

### Oppgave IIb

Lag en figur som viser utviklingen av konsumbasert CO2 per person og totale, konsumbaserte utslipp. Gi en fullstendig og nøyaktig beskrivelse av figuren og forklar hva den viser.

**Hint:** Hvis du bruker `geom_point()` så kan spesifisere én av variablene ved tilvalget `size` i `aes()`.

```{r}
df %>%
  
  mutate(consumption_co2 = consumption_co2/10^3) %>%
  
  filter(country %in% list) %>%
  
  ggplot(aes(x = year, y = consumption_co2, color = country, size = consumption_co2_per_capita)) + geom_point() +
  
  xlim(1990, 2020) +
  
  labs(y = "Consumption co2", x = "Years", title = "Consumption co2 and consumption co2 per capita", subtitle = "Consumption co2 (billion tons) 1990-2020", color = "Country", size = "Consumption co2 per capita") +
  
  theme_bw()


```

*Figuren viser konsumbaserte co2-utslipp og konsumbaserte co2-utslipp per person over tidsperioden 1990 til 2020. Denne viser at Usa har høy konsumbaserte utslipp både totalt og per person, mens kina har etter år 2000 hatt en stor økning i konsumbaserte utslipp totalt og forbigår usa, men har mindre per person. India, Tyskland og Japan er på det nedre siktet på denne figuren i forhold til Usa og Kina. India har mindre per person enn Japan og Tyskland, dette kan ha noe å si med folketallet i hvert av landene, og deres økonomi generelt.*

## III. Produksjon- og konsumbaserte utslipp

Betrakt følgende figur, og gi en konkret forklaring på hva den viser.

```{r}
df %>%

  filter(country %in% c(list)) %>% 

  ggplot(aes(x=year, y=trade_co2_share, color = country)) %>%
  
  + xlim(1990,2020) %>%
  
  + geom_line(size = 1.5) %>%
  
  + geom_hline(yintercept=0) %>%
  
  + labs(x = "Year", y = "Traded CO2 share", title = "Traded CO2 as share of production-based emissions", subtitle = "CO2 embedded in trade. Share of production based emissions. Percent. 1950-2020.
Positive (negative) value indicates net imports (export) of CO2 emissions.", color = "Country") %>%
  
  + theme_bw()
```

*Figuren viser co2-utslipp ved import og eksport i perioden 1990 til 2020 i prosenter. På grafen er det en positiv og negativ y-akse, og denne indikerer eksport og import. negavive verdier er eksport mens positive verdier er import. Land som har mer eksport enn import er Kina og India, samt Usa i perioden 1990 til ca 1997. Landene med mer import kontra eksport er Tyskland og Japan, samt Usa etter 1997. Kina er et land som er kjent for å produsere masse, så det er ikke så overaskende at de også har en større prosentvis eksport enn import. Denne figuren viser at land med høy bnp per person importer prosentvis mer enn de eksporter, og at land med lav bnp per person eksporter mer enn de importer.*
